...
class MNISTDataset(Dataset):
    def __init__(self, filepath, mean = None, std=None):
        ...
        if mean is None:
            mean = x.mean()
        if std is None:
            std = x.std(unbiased=False).clamp_min(1e-8) # 每个batch只关注当前分布，不需要去估计总体分布，不用无偏,后面则是设定最小值防止除以0
        self.mean = mean
        self.std = std                                                     
    ...
    def _load_data(self, filepath):
        ...
        with open(filepath, 'r') as f:  # noqa: UP015
            ...
            for line in f:
                ...
                images.append(list(map(float, sample[1:]))) # 输入是字符串需要处理，不然不能转化为tensor
...
# 定义数据加载器
batch_size = 64
trainfile = r'C:\Users\Anderson\Desktop\RethinkFun深度学习\第八章 - 神经网络\mnist\mnist_train.csv\mnist_train.csv'
